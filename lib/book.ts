...
async function getBlogData(id: string, lang: LangOptions) {
	const originalId = blogSlugs.find((bs: { en: string; vi: string }) => bs[lang] === id)

	if (!originalId) {
		return
	}
	const blogPath = path.join(blogsDirectory, `${originalId['en']}/${lang ?? 'en'}.mdx`)
...
async function getAllBlogsData(lang: LangOptions | undefined) {
	...
	return await Promise.all(blogNames.map(async blogName => {
		const blogPath = path.join(blogsDirectory, blogName, `${lang ?? 'en'}.mdx`)
		const content = fs.readFileSync(blogPath, 'utf-8')
